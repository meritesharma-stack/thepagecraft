        ...
        <div className={styles.myBooks}>
          ...
          {!booksLoading && purchasedBooks.length === 0 && (
            <div className={styles.noBooksMsg}>
              <span>🛒</span>
              <p>You haven't purchased any books yet. <a href="/store" onClick={e => { e.preventDefault(); window.history.pushState({}, '', '/store'); window.dispatchEvent(new PopStateEvent('popstate')) }}>Browse the store →</a></p>
            </div>
          )}
          {!booksLoading && purchasedBooks.length > 0 && (
